...
class Qemu(BaseModel):
    ...
    def get_cpu_specific_args(self) -> list[str]:
        ...
        #if mode == VmQemuConfigMode.UNTRUSTED:
        if mode == VmQemuConfigMode.TDX:
            guest_cid = self.config.qemu_configuration.guest_cid
            args += ["clearcpuid=mtrr", f"-device vhost-vsock-pci,guest-cid={guest_cid}"]
        return args

    def get_kernel_cmdline(self) -> str:
        rootfs_arg = "root=LABEL=rootfs"
        return ' '.join([rootfs_arg] + self.get_cpu_specific_args())
