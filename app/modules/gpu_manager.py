...
class GpuManager:
    def __init__(self):
        ...
        self.lspci_regex_main = re.compile(r'^(?P<pci_address>[a-f\d]+:[a-f\d]+:[a-f\d]+\.[0-7])\W(?P<device_type>.+)\W\[(?P<device_type_id>[a-f\d]+)\]:\W(?P<device_name>.+)\W\[(?P<vendor_id>[a-f\d]+):(?P<device_id>[a-f\d]+)\]')
    ...
    def find_gpu_on_system(self) -> list[Device]:
        self.logger.info(f'searching GPUs on system, with class: `{self.device_class_name}` and vendor id: `{self.vendor_id}`')
        ret = subprocess.run(
    ...
    def replace_driver(self, pci_path: str, driver_name: str) -> None:
        ...
        if not driver_path.is_dir():
            raise Exception(f'failed to replace driver for: `{pci_path}`, reason: driver path `{driver_path}` not found')
        ...
        if not driver_override_path.is_file():
            raise Exception(f'failed to replace driver for: `{pci_path}`, reason: path `{driver_override_path}` not found')
